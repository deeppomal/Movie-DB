  ...
  render() {
    ...
    return (
        <Content style={styles.content}>
            <TouchableOpacity onPress={() => navigation.navigate('tvDetails',{
                data:this.data

            })}>
            <View style={styles.itemView}>
                <Image resizeMethod='resize' style={styles.image} source={{ uri: imageBaseUrl+this.data.poster_path }} />
                <View  style={styles.contentView}>
                    <Text numberOfLines={1} style={styles.title}>{this.data.name}</Text>
